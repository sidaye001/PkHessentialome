...
#ggtitle('Hybrid model score (HMS)') 
...
p.HMS.background <- p.HMS+theme(
  ...
  axis.text = element_text(size = 16),  axis.title=element_text(size=18,margin = margin(t = 1)), legend.background = element_blank())+theme_cowplot()
...
p.HMS.background2 <- p.HMS.background + geom_point(data=point_list, aes(x=geneIndex, y=HMS, group=geneID, fill = geneID), shape = 21, colour = "black", size = 5, stroke = 1) +
  scale_fill_manual(values = c("PKNH_0817000" = "#C63135", "PKNH_0817100" = "#237AB6", "PKNH_0806500" = "lightgrey"),labels = c("PKNH_0817000" ="RIPR","PKNH_0817100"="PKNH_0817100","PKNH_0806500" ="AP2-I"))+
  theme(
    legend.position = c(0.02, 1.02),
    legend.justification = c(0, 1)
  )
#4x4 inches
ggsave(filename = "./Output/Figures/F2/F2B_HMS_schematic3.pdf", plot=p.HMS.background2, width = 4,height = 4, dpi = 300)

###################################fig.S2 For gold plus essential and non-essential genes list###############################
#Input gene symbol
