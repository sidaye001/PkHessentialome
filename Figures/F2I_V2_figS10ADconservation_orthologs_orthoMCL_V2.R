...
new_merged_all_df2$Conservation_group <- factor(new_merged_all_df2$Conservation_group, levels=c('Pk','PvCLADE_PkPcyPv','Plasmodium','Apicomplexan','Eukaryotic'))
...
p11 <- new_merged_all_df2 %>% 
  ...
  scale_x_discrete(labels = c("Pk" = "Pk", "PvCLADE_PkPcyPv" = "PvClade", "Plasmodium" = "Plasmodium","Apicomplexan"="Apicomplexan","Eukaryotic"="Eukaryotic"))  # Change group names on x-axis
#p11
ggsave(filename = "./Output/Figures/F2/F2_FIS_conservation_depth.pdf", plot=p11, width = 4,height = 4, dpi = 300)
...
p2 <- ggplot(Proportion, aes(x = Var1, y = Freq, fill=Var1)) +
  ...
  geom_text(aes(label = Freq), position = position_dodge(width = 0.8), vjust = -0.5, size = 5) +  # Add count numbers on top of bars
  xlab("") +
  ylab("No. of genes") +
  ggtitle("") + theme_cowplot() + theme(
    plot.title = element_text(color="black", size=14, face="bold"), 
...
ggsave(filename = "./Output/Figures/F2S/F2_conservation_group_statistics_bar.pdf", plot=p2, width = 4,height = 4.8, dpi = 300)
...
GO_plot <- function(filtered.Go2){
  ...
  pp <- ggplot(filtered.Go2, aes(x = log(`Fold enrichment`), y = -log(`P-value`), label = Name)) +
    ...
    guides(size = guide_legend(override.aes = list(fill = "black")))+# Change "black" to your desired fill color
    theme(strip.text = element_text(size = 16))+ylim(c(0,80))
