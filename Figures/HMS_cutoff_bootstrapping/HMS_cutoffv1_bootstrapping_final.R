...
#To check the quantile of No. of TTAA
qq <- quantile(tr.dat$num_TTAA, probs = c(0.20, 0.80))
print(qq)

#To classify the No. of TTAA into ">20", "4-20", "<4" and merged all the candidate as "combined"
...
tr.dat$TTAA_class <- factor(tr.dat$TTAA_class, 
                            levels = c(paste('<', qq[1]), 
                                       ...
                                       paste('>', qq[2]), 
                                       'combined'))
#To check the HMS distribution for each category
plot(tr.dat$TTAA_class, tr.dat$Pk.HMS)

#To assigned class "essential" to "Gold Essential High", "Gold Essential Medium"
#To assigned class "NonEssential" to "Gold non-essential High", "Gold non-essential Medium"
...
#To filter out training data
trainin.data <- tr.dat %>% dplyr::filter(TTAA_class != 'combined' & class != 'NA')


## Quantiles based on training class, 0.2 and 0.75 quantiles on classes of "essential" and "Nonessential"
...
extendedgoldplus_HMS
ggsave(filename = "./Output/Figures/F2S/extendedgoldplus_HMS.pdf", plot=extendedgoldplus_HMS, width =4,height = 4, dpi = 300)
...
#########Model1: To find the cutoff based on two separate distribution##########
#########Model1: To find the cutoff based on two separate distribution##########
#########Model1: To find the cutoff based on two separate distribution##########
...
hist(subset(HMS, cl == 1)$y, breaks = 0:25/25, freq = FALSE,
     col = 'lightpink', main = "", xlab = "HMS", ylim = c(0, 10),family = "sans", cex.axis = 1.4, cex.lab = 1.6)

hist(subset(HMS, cl == 2)$y, breaks = 0:25/25, freq = FALSE,
     col = 'orange', main = "", xlab = "HMS", ylim = c(0, 10),family = "sans", cex.axis = 1.4, cex.lab = 1.6,add = TRUE)
...
c11 <- qbeta(0.8,  shape1 = a[1], shape2 = b[1])
c22 <- qbeta(0.8,  shape1 = a[2], shape2 = b[2])
...
## For normal distribution 2sd away from mean covered 95% of the population
...
legend(0.5, 9, legend=c("essential", "dispensable"),
...
## Proportions of gene classification
