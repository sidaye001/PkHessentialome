...
MFS_cm2_long <- pivot_longer(MFS_cm2, everything(), names_to = "Samples", values_to = "Value")
###To remove 0s
MFS_cm2_long <- MFS_cm2_long[MFS_cm2_long$Value!=0,]

out.dir <- "./Output/MFS/figs/"
# Create boxplot
ggplot(MFS_cm2_long, aes(x = Samples, y = Value)) +
  geom_boxplot() +
  labs(x = "Samples", y = "Value") +
  theme_minimal()

ggsave(filename = paste(out.dir,"sum_reads_category_timepoints_day9_14_19_goodsample", '.pdf',sep = ""), width = 4,height = 6, dpi = 300)

#####Loess normalization
MFS_cm_normalized <- normalize.loess(MFS_cm2)
...
ggplot(MFS_cm2_long, aes(x=Samples, y=Value)) +
  ...
  labs(y='Sum of Reads',x='',title = '')+theme(
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 0.5, vjust = 0.5, face = "bold"))+theme(panel.grid = element_blank())
