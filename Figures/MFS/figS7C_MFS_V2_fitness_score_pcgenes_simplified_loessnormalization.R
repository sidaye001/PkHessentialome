...
#########Pipe in the normalized transposon count matrix after bg noise removal##########
#########Pipe in the normalized transposon count matrix after bg noise removal##########
#########Pipe in the normalized transposon count matrix after bg noise removal##########
...
MFS_cm2_cpm_log2 <- log2(as.matrix(MFS_cm2)+1)
MFS_cm3_log2 <- normalizeBetweenArrays(MFS_cm2_cpm_log2, method = "cyclicloess")
MFS_cm2 <- 2^MFS_cm3_log2

###To make sure every count is non-negative
min(MFS_cm2)

MFS_cm2 <- as.data.frame(MFS_cm2)
MFS_cm2$Total <- rowSums(MFS_cm2)
#############################MFSg calculation#############################
#############################MFSg calculation#############################
#############################MFSg calculation#############################
###Get the TTAA ID and location info from transposon count matrix
...
#######Step3: relative abundance of mutant, the normalized reads number of mutant m targeting gene g on CDS in all/each transfection pool
...
#########################mode2: To calculate MFS for every TPN at each timepoint for each gene##############
...
write.xlsx(merged_MFS,'./Output/MFS/merged_MFS_pcgenes_loess_normalization.xlsx')

########################To fit a linear regression model across three time points###################
merged_MFS <- read.xlsx('./Output/MFS/merged_MFS_pcgenes_loess_normalization.xlsx')
...
write.xlsx(regression_results,'./Output/MFS/MFS_regression_trending_results_pcgenes_loess_normalization.xlsx')
write.xlsx(combined_data,'./Output/MFS/MFS_scores_results_pcgenes_loess_normalization.xlsx')

#######################For direct plotting########################
#######################For direct plotting########################
#######################For direct plotting########################
combined_data <- read.xlsx('./Output/MFS/MFS_scores_results_pcgenes_loess_normalization.xlsx')
...
regression_results2$MFS.slope <- round(regression_results2$MFS.slope,3)

#######################To generate figures in batch by inputting gene list################################


################To batch generating figures in different gene list################################
################To batch generating figures in different gene list################################
################To batch generating figures in different gene list################################
...
non_essential_geneslist <- read.table("/Users/sidaye/Documents/R/Tnseq/202212_Novaseq/Input/Nonessential_geneslist_with_confidence_v2.txt")

out.dir <- "./Output/Figures/F2/MFS/loess/TCA_core/"
Input_gene_list <- TCA_core

out.dir <- "./Output/Figures/F2/MFS/loess/TCA_entry/"
Input_gene_list <- TCA_entry

max_MFS <- max(combined_data$Value)
max_MFS <- 0.25
#####Min MFS should be the minimum of MFS bottom line 
#min_MFS <- min(MFS_bottom_line$MFS_bottom_line)
min_MFS <- min(regression_results2$MFS_bottom_line)
min_MFS <- -4
...
#4X4inches
#######################To generate single gene's figure################################
#######################To generate single gene's figure################################
#######################To generate single gene's figure################################
...
###Check discrepancy between OIS and HMS
geneName <- "PKNH_0618900"
geneName <- "PKNH_1404000"
geneName <- "PKNH_0005800"
geneName <- "PKNH_0403700"
geneName <- "PKNH_0503200"
geneName <- "PKNH_0402400"



gene_name <-"HEUL" #HECT-like E3 \n ubiquitin ligase
...
###Check discrepancy between OIS and HMS
gene_name <- "SICAvar, type I"
gene_name <- "peptidase"
gene_name <- "roadbblock"
gene_name <- "unknown function"
gene_name <- "tRNA modification \n GTPase"
gene_name <- "SNRPD2"
...
#selected_df$Timepoint <- as.numeric(selected_df$Timepoint)
#lm(Value~Timepoint,data=selected_df)
#test <- selected_df%>%group_by(Timepoint)%>%summarize(mean=mean(Value), sd=sd(Value))
...
ggsave(filename = paste0("./Output/Figures/F2S/MFS_trending/figS7c_FIS_",geneName,".pdf"), plot=pp, width = 4,height = 4, dpi = 300)
###4X4inches
ggsave(filename = paste0("./Output/Figures/F2S/MFS_trending/figS7c_FIS_",geneName,".pdf"), plot=pp, width = 4,height = 4.2, dpi = 300)
##############Trending lines for gene list#############
##############Trending limes for gene list#############
##############Trending lines for gene list#############
TCA_core <- read.xlsx('./Input/Combined_gene_lists.xlsx',sheet=2)
TCA_entry <- read.xlsx('./Input/Combined_gene_lists.xlsx',sheet=3)
ETC<- read.xlsx('./Input/Combined_gene_lists.xlsx',sheet=4)
...
selected_df_list3<- combined_data[combined_data$geneID%in%non_essential_geneslist$V1,]
selected_df_list4<- combined_data[combined_data$geneID%in%TCA_entry$PkGene,]
selected_df_list5<- combined_data[combined_data$geneID%in%ETC$PkGene,]
selected_df_list6<- combined_data[combined_data$geneID%in%mito$PkGene,]
selected_df_list7<- combined_data[combined_data$geneID%in%AP2$PkGene,]
selected_df_list8<- combined_data[combined_data$geneID%in%Pkinase$PkGene,]
selected_df_list9<- combined_data[combined_data$geneID%in%Proteosome$PkGene,]
selected_df_list10<- combined_data[combined_data$geneID%in%Ribosome$PkGene,]
selected_df_list11<- combined_data[combined_data$geneID%in%Apicoplast$PkGene,]



TCA_regression_results <- regression_results[regression_results$geneID%in%TCA_core$PkGene,]
TCA_entry_regression_results <- regression_results[regression_results$geneID%in%TCA_entry$PkGene,]
essen_regression_results <- regression_results[regression_results$geneID%in%essential_geneslist$GeneID,]
nonessen_regression_results <- regression_results[regression_results$geneID%in%non_essential_geneslist$V1,]

genelist_name <- "Apicoplast genes"
out.dir <- "./Output/Figures/F2/MFS/loess/"
ggplot(selected_df_list12, aes(x = Timepoint, y = Value, color = geneID)) +
  stat_smooth(aes(group = geneID), method = "lm", se = TRUE, linetype = "solid", color = "black") +
  labs(title = genelist_name,
       x = "Timepoint", y = "MFS",
       color = "TPN") +
  theme_cowplot() +
  scale_x_discrete(labels = c("1" = "t1", "2" = "t2", "3" = "t3"))+
  theme(legend.title = element_blank(), axis.text = element_text(size = 14), 
        axis.title = element_text(size = 18),
        plot.title = element_text(hjust = 0.5, size = 16))+ylim(c(-3,0))

ggsave(filename = paste(out.dir, paste0(genelist_name, '.pdf')), width = 4,height = 4, dpi = 300)
...
ggplot(selected_df_list3, aes(x = Timepoint, y = Value, color = geneID)) +
  stat_smooth(aes(group = geneID), method = "lm", se = TRUE, linetype = "solid", color = "black") +
  labs(title = "Gold plus nonessential genes",
       x = "Timepoint", y = "MFS",
       color = "TPN") +
  theme_cowplot() +
  scale_x_discrete(labels = c("1" = "t1", "2" = "t2", "3" = "t3"))+
