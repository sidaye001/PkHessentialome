...
#######Mapping Pb fitness slow genes and plot correlation between Pb.RGR and HMS, Pb.RGR and FIS
####To input the Pb data
Pb.RGR <- read.csv("./Input/Comparative_essen/Pb_growth_rate_score.csv")
Pb.RGR.slow <- Pb.RGR%>%dplyr::filter(phenotype=='Slow')

df2 <- df2%>%dplyr::mutate(Pb.phenotype=ifelse(GeneID.Pb_ANKA%in%Pb.RGR.slow$current_version_ID,1,0))
df2.Pb.slow <- df2%>%filter(Pb.phenotype==1)

################################The background is all Pk genes betwee Pk and Pb#################
p.mapping.Pb.RGR <- ggplot(data = df2, mapping = aes(x = HMS, y = MFS.slope)) +
  geom_point(data = df2, aes(color = Pb.phenotype >0), shape = 16,  size = 2) +
  scale_color_manual(values = c("TRUE" = "#E3770C","FALSE" = "grey"), guide = FALSE) +
  theme(
  ...
  geom_vline(xintercept = kneepoints2, linetype = "dashed", color = "#C63135",linewidth=1.2,alpha = 1)+
  geom_vline(xintercept = kneepoints3, linetype = "dashed", color = "#237AB6",linewidth=1.2,alpha = 1)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "black",linewidth=1,alpha = 1)+
  ylim(c(-0.5, 0.5))+
  geom_point(data = df2.Pb.slow, shape = 16,  size = 2, color="#E3770C")

p.mapping.Pb.RGR 

MapPbslow_HMS_FIS1 <- ggMarginal(p.mapping.Pb.RGR, type = c("density"),groupColour = TRUE, groupFill = TRUE)
ggsave(filename = "./Output/Figures/review/MappingPbslow_AllPkgenes_background.pdf", plot=MapPbslow_HMS_FIS1, width = 4,height = 4, dpi = 300)
################################The background is all orthologous genes betwee Pk and Pb#################
df22 <- df2[!is.na(df2$GeneID.Pb_ANKA),]
p.mapping.Pb.RGR2 <- ggplot(data = df22, mapping = aes(x = HMS, y = MFS.slope)) +
  geom_point(data = df22, aes(color = Pb.phenotype >0), shape = 16,  size = 2) +
  scale_color_manual(values = c("TRUE" = "#E3770C","FALSE" = "grey"), guide = FALSE) +
  theme(
    legend.key = element_rect(fill = "transparent", colour = "transparent"), 
    legend.text = element_text(size=14),
...
MapPbslow_HMS_FIS2 <- ggMarginal(p.mapping.Pb.RGR2, type = c("density"),groupColour = TRUE, groupFill = TRUE)
ggsave(filename = "./Output/Figures/review/MappingPbslow_PkPbortho_genes_background.pdf", plot=MapPbslow_HMS_FIS2, width = 4,height = 4, dpi = 300)
################################The background is all orthologous genes betwee Pk and Pb#################
